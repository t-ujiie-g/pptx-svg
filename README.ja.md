...
| オプション | 型 | 説明 |
|-----------|------|------|
| `measureText` | `(text, fontFace, fontSizePx) => number` | テキスト幅計測のカスタム関数。デフォルトは Canvas 2D。 |
| `fontFallbacks` | `Record<string, string[]>` | フォントフォールバックマッピング。組み込みデフォルトとマージされます。 |
...
- **WMF 画像** — ブラウザでデコード不可（EMF は内蔵コンバータで対応済み）
