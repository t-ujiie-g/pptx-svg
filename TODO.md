...
- [ ] テキストワープ (`a:prstTxWarp`) — パース + データ保持 + 可能な範囲で SVG 近似表示
- [ ] テキストアウトライン (`a:rPr/a:ln`) — パース + データ保持 + SVG `stroke` で近似表示
- [ ] テキストグラデーション塗り (`a:rPr/a:gradFill`) — パース + データ保持 + 先頭色でフォールバック表示

---

## 3. データ保持 — レンダリング限定、round-trip 必須 [P2]
...
| **P2** | テキスト高度 (ワープ/アウトライン/グラデーション) | 近似 | 完全 | 未着手 |
