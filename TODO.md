...
**次セッションは E6.5（コピー&ペースト・スライド跨ぎ）から着手**（「開発を続けて下さい」で再開可能）。
...
- [ ] **E6.5 シェイプのコピー&ペースト（スライド跨ぎ）** (P5) — `getShapeSpec(slideIdx, shapeIdx)`（移植可能な JSON/内部XML で取得）/ `insertShapeSpec(slideIdx, spec, dxEmu?, dyEmu?)`（別スライドへ復元挿入）。画像入りは `.rels`/メディア再リンクが必要 → 既存 `add_picture_shape` + `[Content_Types].xml`/`.rels` 自動更新ロジックを再利用。
...
| **E5** | 画像追加/差替/削除 | — | — | 未着手 |
| **E6** | 直接操作編集 (Undo/Redo, インライン文字, z-order, 一括transform, コピペ, セル編集) | — | — | **一部完了** (E6.1〜E6.4) |
