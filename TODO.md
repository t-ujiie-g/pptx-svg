...
### データ保持・フォールバック
- [x] SmartArt: cached drawing (`dsp:drawing`) ネイティブ描画 + mc:Fallback シェイプ描画 + 原 graphicFrame 保持 (round-trip)
  - 素の `p:graphicFrame`(uri=diagram) を `relIds@r:dm → data → dsp:dataModelExt@relId → drawing` の連鎖で解決し、`dsp:` 図形を描画
  - 文字色: cached drawing が色を持たない場合 `cs`(colors) パートの txClrLst から解決 / 空 bodyPr は垂直中央寄せ
...
- [x] **下線色のデータ往復** — 下線色(`a:uFill`/`a:uLn`)を `TextRun.underline_color` にパース → `data-ooxml-underline-color` round-trip → `<a:uFill>` serialize。下線スタイル値(`u`=dbl/wavy/…)は従来通り保持。副次的に svg_parser の `color_or_none` プレフィックス欠落バグ（outline/3D/blip duotone 色の round-trip 欠落）も修正
- [ ] **下線スタイル/色の「描画」** — double/dotted/wavy や色は**ブラウザが SVG `<text>` の `text-decoration-style`/`-color` を一切無視する**ため CSS では描画不可（Chromium で検証済み: プレゼン属性・CSSプロパティ・CSSショートハンドいずれも無効）。描画には下線を独立図形（`<line>`/`<path>`：double=2本、dotted/dashed=dasharray、wavy=波形path）として描く必要があり、ラン毎の x位置・幅算出（[[テキストハイライト]] と共通のレイアウト改修）が前提。データは保持済み
- [ ] ~~OpenType フォント機能 (numSpacing/numForm)~~ — **対象外**: これらは WordprocessingML(`w:`) の属性で、PPTX の DrawingML(`a:rPr`/`a:latin`=CT_TextFont は typeface/panose/pitchFamily/charset のみ) には存在しない
- [ ] **テキストハイライト（a:highlight）** — 蛍光ペン背景色。SVG はテキスト背景を持たないため、ラン毎の背景 `<rect>` 配置（ラン x 位置の累積計測）が必要で、現行のフローレイアウトへの統合が重い。価値は高いが要レイアウト改修（上記「下線スタイル/色の描画」と同じラン箱算出基盤を共有）
...
- [x] **SmartArt の cached drawing 描画** — `dsp:drawing`（`ppt/drawings/drawingN.xml`）の静的図形を描画。下記 SmartArt 仕様ギャップは残存（フル DiagramML エンジン未実装のため意図的な制限）:
  ...
  - [ ] **カラートランスフォームのノード別サイクリング** — `cs`(colors) の `fillClrLst`/`linClrLst`/`txClrLst` の `meth="cycle/span"` 未対応。文字色は先頭 1 色を一律適用、塗り/線は cached drawing にベイク済み前提
  - [ ] **データモデル(dm) を使った再レイアウト・編集** — `dgm:dataModel`(pt/cxn) は未使用。静的な図形グループとして扱う（原 graphicFrame は `ole_xml` 保持）
...
### E6: 直接操作編集（consumer `pptx-svg-ai-demo` requested 2026-06）

PowerPoint / Google スライド級の直接操作編集に必要な基盤プリミティブ。
依頼文書: `pptx-svg-editing-api-request.md`（P1〜P6）。優先度順に E6.1〜E6.6 として管理。
**E6（直接操作編集 P1〜P6）は全て完了。** mergeCells/splitCells（結合）が将来候補。

- [x] **E6.1 Undo / Redo（編集履歴）** (P1, 最重要) — `undo()`/`redo()`/`canUndo()`/`canRedo()`/`beginBatch()`/`endBatch()`/`clearHistory()` + `maxHistory` option。全ミューテーション API に checkpoint。統一 DocSnapshot 方式（TS files 差分 + Wasm modified スライド OOXML）。新 Wasm export `restore_slide_ooxml` + 共有 `resolve_slide_data` ヘルパー。`undo/redo` は `{slides, slideCount}` の JSON を返す。MoonBit 冪等性テスト + Node 統合テスト Test 37–43。
- [x] **E6.2 インライン文字編集の土台** (P2) — 本セッションで完了。ダブルクリックで直接タイプ（IME 込み）するための幾何情報 + 範囲置換プリミティブ。3 API すべて Wasm。`replaceTextRange` は E6.1 履歴に統合（undo 可能）。
  - [x] `getTextLayout(slideIdx, shapeIdx)` → JSON: `{box, lines:[{paraIdx,y,h,runs:[{paraIdx,runIdx,x,w,runCharStart,paraCharStart,chars:[{x,w}]}]}]}`（EMU、per-char advance）。`render_text` の `solve_text_autofit` 抽出 + `wrap_paragraph` 共有で行数/run数一致を保証。v1 は水平 LTR（縦書き/warp/math/多段組は box のみ）。
  - [x] `hitTestText(slideIdx, shapeIdx, xEmu, yEmu)` → JSON: `{paraIdx, runIdx, charOffset, paraOffset}`
  - [x] `replaceTextRange(slideIdx, shapeIdx, startPara, startChar, endPara, endChar, newText)`（段落レベルオフセット。run 分割で両側の書式維持。`\n` は段落分割／段落跨ぎはマージ）
  - [ ] (任意・未) `hitTestShape(slideIdx, xEmu, yEmu)` → shapeIdx（透明塗り・z-order 考慮。当面 consumer は DOM `closest()` で代替可）
  - [ ] (将来) 縦書き/warp/math/多段組の per-char 厳密幾何（v1 は box のみ概算）
- [x] **E6.3 z-order（重なり順）操作** (P3) — 本セッションで完了。`bringToFront`/`sendToBack`/`bringForward`/`sendBackward`。z-order = shape 配列順（末尾＝最前面）。含有配列（slide.shapes / group.children）内で並べ替え、`OK:<new_composite_idx>` を返す。E6.1 履歴に統合（undo 可能）。汎用 `reorderShape` は4メソッドで要件充足のため見送り。
- [x] **E6.4 複数シェイプの一括 transform** (P4) — 本セッションで完了。`updateShapesTransform(slideIdx, items:[{shapeIdx,x,y,cx,cy,rot}])`。全 shapeIdx を検証してから適用（原子的・部分適用なし）・1 undo 単位。失敗時は履歴に残さない guarded checkpoint（`commitCheckpoint`）を追加。
- [x] **E6.5 シェイプのコピー&ペースト（スライド跨ぎ）** (P5) — 本セッションで完了。`getShapeSpec`（シェイプの OOXML + 参照画像を base64 内包した移植可能 JSON）/ `insertShapeSpec`（別スライドへ貼付、メディア再追加 + rId 再リンク、オフセット対応、undo 可能）。MoonBit: `serialize_shape` pub 化 + `parse_shapes_xml` + `get_shape_ooxml`/`add_shape_from_ooxml`。TS: `base64ToBytes`。**v1 制約**: chart は非対応（serialize 不可）、OLE/SmartArt は画像メディアのみ再リンク。
- [x] **E6.6 テーブルセル編集** (P6, 既存 E3 と整合) — 本セッションで完了。`updateTableCellText`（セルの first-run 書式を継承）+ `addTableRow`/`deleteTableRow`/`addTableColumn`/`deleteTableColumn`（最低1行/1列保持）。`TableCell::empty()` 追加。すべて undo 可能。**v1 制約**: 結合セル（gridSpan/rowSpan/hMerge/vMerge）は span 調整しないため、結合を含む表の行列操作は構造が崩れる可能性あり。`mergeCells`/`splitCells` は見送り（将来タスク）。
  - **→ これで E6（直接操作編集 P1〜P6）完了。** consumer リクエスト `pptx-svg-editing-api-request.md` の全項目に対応。

> **スコープ外（consumer 側で対応 / 既に可能）**: ドラッグ移動・リサイズ/回転ハンドル UI（`updateShapeTransform` が rot 対応済み）、スナップ/スマートガイド（`getAllShapes` の幾何から算出）、キーボードショートカット・右クリックメニュー・スライドサムネイル一覧（`renderSlideSvg` で描画）。
...
| **E3** | テーブル行列操作 / セル編集 | — | — | **完了** (E6.6 で実装、結合は将来) |
| **E4** | チャートデータ編集 / シリアライズ | — | — | 未着手 |
| **E5** | 画像追加/差替/削除 | — | — | 未着手 |
| **E6** | 直接操作編集 (Undo/Redo, インライン文字, z-order, 一括transform, コピペ, セル編集) | — | — | **完了** (E6.1〜E6.6) |
