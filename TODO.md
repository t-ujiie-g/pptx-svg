...
ChartML (ECMA-376 Part 1 Chapter 21) パーサー + SVG レンダラー。

### 実装済み
- [x] 基盤: `c:chartSpace` パース / プロットエリア / 軸 (catAx/valAx) / 凡例 / タイトル
- [x] チャート種別: 棒(縦・横) / 折れ線 / 円 / ドーナツ / 散布 / 面 / レーダー
- [x] 系列データ: numCache/strCache からの読み取り / カテゴリラベル
- [x] レイアウト: 横棒チャートの軸反転 / 円・ドーナツの軸なしレイアウト
- [x] 凡例: 系列タイトル表示 / 円・ドーナツはカテゴリ名表示
- [x] 整数三角関数: cos/sin ルックアップテーブル (1°刻み × 1000 精度)
- [x] データラベル (`c:dLbls`) — showVal/showCatName/showSerName/showPercent パース + 棒/折れ線/円/ドーナツ SVG レンダリング
- [x] データポイント個別書式 (`c:dPt`) — パース + 円/ドーナツ/棒 per-point 色反映
- [x] 系列書式 (`c:spPr` 色反映) — fill/stroke 色パース + レンダリング反映
- [x] テーマカラー連動 — accent1-6 からチャートカラーパレット自動構築 + ダーク variant

### 未実装
- [ ] チャート種別追加: バブル / 株価 / 等高線
- [ ] 複合チャート (複数チャート種別を1プロットエリアに)、3D チャート (2D 投影で近似)
...
| **P2** | チャート基本 (棒/線/円/ドーナツ/散布/面/レーダー) | **完了** |
| **P2** | チャート高度 (データラベル/dPt/系列色/テーマカラー) | **完了** |
| **P2** | チャート残 (バブル/株価/等高線/トレンドライン) / テキスト高度 | 未着手 |
