...
- [x] **テキスト高度機能**: テキストワープ (prstTxWarp) + テキストアウトライン + テキストグラデーション/パターン塗り
- [x] **ライブラリ公開準備**: Wasm 自動解決 (`init()` 引数なし対応) + npm パッケージ整備 + CI/CD + ドキュメント + examples

---

## 1. データ保持 — レンダリング限定、round-trip 必須 [P2]
...
- [x] **SmartArt** (`dgm:*` DiagramML) — フォールバック描画 (`mc:AlternateContent` → `mc:Fallback` シェイプ) で表示 + `mc:Choice` 原 XML 保持
- [x] **OLE / 埋め込み** (`p:oleObj`) — フォールバック画像で表示 + 原 XML 保持
- [x] **メディア** (ビデオ/オーディオ) — ポスターフレーム画像で表示 + `a:videoFile`/`a:audioFile` 原 XML 保持

### バイナリ保持

- [x] **EMF** — 内蔵コンバータで SVG 変換 + バイナリ round-trip 保持
- [x] **WMF** — ZIP 内バイナリ保持 + rels 保持 (ブラウザでデコード不可)
- [x] **TIFF** — ZIP 内バイナリ保持 + rels 保持 (ブラウザ `<img>` 非対応)

### パース + データ保持 (簡略表示)

- [x] **数式** (OMML `m:oMath`) — 原 XML 保持 + `m:t` からプレーンテキスト抽出でフォールバック表示
- [x] **埋め込みフォント** (`a:fontScheme` + fontdata) — ZIP 内バイナリ保持 + システムフォントフォールバック
...
| **P2** | SmartArt (mc:Fallback 描画 + mc:Choice 保持) | FB描画 | 完全 | **完了** |
| **P2** | OLE / 埋め込み (フォールバック画像 + 原 XML 保持) | FB画像 | 完全 | **完了** |
| **P2** | メディア (ポスターフレーム + 原 XML 保持) | FB画像 | 完全 | **完了** |
| **P2** | EMF (SVG 変換) / WMF・TIFF (バイナリ保持) | EMF変換 | 完全 | **完了** |
| **P2** | 数式 OMML (テキストフォールバック + 原 XML 保持) | テキスト | 完全 | **完了** |
| **P2** | 埋め込みフォント (バイナリ保持 + システムフォールバック) | FB | 完全 | **完了** |
