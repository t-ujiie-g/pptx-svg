...
- [ ] **シェイプの追加** — シェイプ ID 採番 + `.rels` 更新（画像等の場合）
- [ ] **シェイプの削除 API** — SVG round-trip で削除は可能だが、専用メソッドなし
- [ ] **シェイプの複製** — ID 再採番 + メディア参照の共有/複製ハンドリング
- [ ] **グラデーション/パターン塗りつぶし編集 API** — 現在は `updateShapeFill()` でソリッド fill のみ
- [ ] **エフェクト編集 API** — 影・グロー・リフレクション等の個別プロパティ変更
...
| **E2** | シェイプ追加/削除/複製 / 塗り編集 / エフェクト編集 | — | — | 未着手 |
