...
**次セッションは E6.3（z-order）から着手**（「開発を続けて下さい」で再開可能）。
...
- [ ] **E6.3 z-order（重なり順）操作** (P3) — `bringToFront`/`sendToBack`/`bringForward`/`sendBackward`（または汎用 `reorderShape(slideIdx, shapeIdx, newZIndex)`）。`spTree` 子要素の並べ替え。戻り値に**新 shapeIdx** を含め consumer が再選択しやすくする。
...
| **E5** | 画像追加/差替/削除 | — | — | 未着手 |
| **E6** | 直接操作編集 (Undo/Redo, インライン文字, z-order, 一括transform, コピペ, セル編集) | — | — | **一部完了** (E6.1, E6.2) |
