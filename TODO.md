...
**次セッションは E6.6（テーブルセルテキスト編集）から着手**（「開発を続けて下さい」で再開可能）。
...
- [ ] **E6.6 テーブルセル編集** (P6, 既存 E3 と整合) — 最低限 `updateTableCellText(slideIdx, shapeIdx, row, col, text)`。余力で `addTableRow`/`deleteTableRow`/`addTableColumn`/`deleteTableColumn`/`mergeCells`。
...
| **E5** | 画像追加/差替/削除 | — | — | 未着手 |
| **E6** | 直接操作編集 (Undo/Redo, インライン文字, z-order, 一括transform, コピペ, セル編集) | — | — | **一部完了** (E6.1〜E6.5) |
