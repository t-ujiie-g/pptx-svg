...
**次セッションは E6.2 から着手**（「開発を続けて下さい」で再開可能）。
...
- [ ] **E6.2 インライン文字編集の土台** (P2) — ダブルクリックで直接タイプ（IME 込み）するための幾何情報 + 範囲置換プリミティブ。
  - `getTextLayout(slideIdx, shapeIdx)` → JSON: `{box, lines:[{y,h,runs:[{paraIdx,runIdx,x,w,chars:[{x,w}]}]}]}`（autoFit 縮小後の実フォントサイズ反映、per-char advance。既存 `MeasureTextFn` 再利用）
  - `hitTestText(slideIdx, shapeIdx, xEmu, yEmu)` → JSON: `{paraIdx, runIdx, charOffset}`
  - `replaceTextRange(slideIdx, shapeIdx, startPara, startChar, endPara, endChar, newText)`（run 途中は分割し両側の書式維持。プレーンテキスト差し替えで可）
  - (任意) `hitTestShape(slideIdx, xEmu, yEmu)` → shapeIdx（透明塗り・z-order 考慮。当面 consumer は DOM `closest()` で代替可）
...
| **E5** | 画像追加/差替/削除 | — | — | 未着手 |
| **E6** | 直接操作編集 (Undo/Redo, インライン文字, z-order, 一括transform, コピペ, セル編集) | — | — | **一部完了** (E6.1) |
