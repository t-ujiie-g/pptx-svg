...
- [ ] **アニメーション/トランジション round-trip 保持** — SlideData に `transition_xml` / `timing_xml` フィールドを追加し、パース→保持→シリアライズで欠損を防ぐ。レンダリングは不要（静的表示のみ）
- [ ] **隠しスライド検出** — `<p:sld show="0">` をパースし、SlideData に `hidden: Bool` フィールドを追加。API で取得可能にする

### P2: 数式レンダリング (OMML → SVG)

現在はプレーンテキストフォールバックのみ。OMML の構造を SVG の配置に変換することで視覚的に正しい数式表示を実現する。

- [ ] **分数** (`m:f`) — 分子/分母を上下に配置 + 分数線を描画
- [ ] **上付き/下付き** (`m:sSup` / `m:sSub` / `m:sSubSup`) — baseline シフトで配置
- [ ] **根号** (`m:rad`) — √ 記号 + 上線を SVG path で描画
- [ ] **大型演算子** (`m:nary`) — ∫ / Σ / Π + 上下限の配置
- [ ] **括弧** (`m:d`) — 伸縮する括弧/中括弧の描画
- [ ] **アクセント** (`m:acc`) — 上付き記号 (ˆ, ˜, ¯ 等)
- [ ] **行列** (`m:m`) — グリッド配置
- [ ] **オーバー/アンダーバー** (`m:bar`) — 上線/下線の描画

### P3: テキストワープ視覚レンダリング

現在はデータ保持のみ。prstTxWarp のプリセット (arch, wave, chevron 等) に沿ったパス上テキスト配置を実装する。

- [ ] **テキストワープレンダリング** — ワープパス計算 + SVG `<textPath>` によるカーブ沿い配置

### P3: WMF → SVG 変換

EMF コンバータと同様の手法で WMF (16-bit GDI) をパースし SVG に変換する。

- [ ] **WMF コンバータ** — WMF レコードパース + GDI 状態管理 + SVG パス/テキスト出力
...
- [ ] **blur エフェクト** (`a:blur`) — SVG `filter: blur()` で容易に対応可能
- [ ] **プリセットシャドウ** (`a:prstShdw`) — 現在は `a:outerShdw` のみ対応
- [ ] **fillOverlay** (`a:fillOverlay`) — 塗りつぶしのオーバーレイ合成

### P4: その他

- [ ] **均等割り付け** (align="just") — SVG にはネイティブの justify がないため、ワード間スペーシングで近似
- [ ] **TIFF デコーダ** — LZW/JPEG 圧縮対応が必要で難易度高。TIFF は PPTX 内で稀
...
| **P1** | アニメーション/トランジション round-trip 保持 | — | 保持 | 未着手 |
| **P1** | 隠しスライド検出 | — | — | 未着手 |
| **P2** | 数式レンダリング (OMML → SVG) | 数式 | 完全 | 未着手 |
| **P3** | テキストワープ視覚レンダリング | ワープ | 完全 | 未着手 |
| **P3** | WMF → SVG 変換 | 変換 | 完全 | 未着手 |
| **P3** | Office 2016+ チャート (cx namespace) | 完全 | 完全 | 未着手 |
| **P4** | 追加エフェクト (blur/prstShdw/fillOverlay) | 完全 | 完全 | 未着手 |
