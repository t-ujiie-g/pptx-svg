...
**次セッションは E6.4（複数シェイプ一括 transform）から着手**（「開発を続けて下さい」で再開可能）。
...
- [ ] **E6.4 複数シェイプの一括 transform** (P4) — `updateShapesTransform(slideIdx, items:[{shapeIdx,x,y,cx,cy,rot}])`。原子的・1 undo 単位。※ E6.1 の `beginBatch/endBatch` で consumer 側代替も可能なため優先度は下げて良い。
...
| **E5** | 画像追加/差替/削除 | — | — | 未着手 |
| **E6** | 直接操作編集 (Undo/Redo, インライン文字, z-order, 一括transform, コピペ, セル編集) | — | — | **一部完了** (E6.1, E6.2, E6.3) |
